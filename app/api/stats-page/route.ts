...
export async function GET(request: NextRequest) {
  try {
    ...
    playerStatsMap.forEach(playerStats => {
      ...
      const goalsPoints = playerStats.total_goals * GOALS_POINT
      const assistsPoints = playerStats.total_assists * ASSIST_POINT
      const savesPoints = playerStats.total_saves * SAVES_POINT
      // Defenders get more points per clean sheet
      const cleanSheetsPoints = playerStats.is_defender
        ? playerStats.total_clean_sheets * CLEAN_SHEET_POINT_DEFENDER
        : playerStats.total_clean_sheets * CLEAN_SHEET_POINT_OTHER
      const ownGoalsPenalty = playerStats.total_own_goals * OWN_GOAL_PENALTY // Negative points
